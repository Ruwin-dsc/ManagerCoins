...
module.exports = {
    ...
    async execute(client, message) {
        ...
        try {
          message.channel = message.channel || await message.author.createDM()
          command.execute(client, message, args)
          cooldown.set(`${message.author.id}_${command.name}`, Date.now() + (command.cooldown || 3000))
          setTimeout(() => { cooldown.delete(`${message.author.id}_${command.name}`)}, command.cooldown || 3000)
        } catch (error) {
